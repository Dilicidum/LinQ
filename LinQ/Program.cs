...
namespace ConsoleApplication8
{
    ...
    public class Student
    {
        ...
        public int YearApplying { get; set; }
    ...
    class Program
    {
        static void Main(string[] args)
        {
...

            {
                ...
                List<string> list = new List<string> { "Ira", "Katya", "Tom", "Kostya"};
                int result = list.Sum(i => i.Length);
...

            {
                ...
                //TODO
                List<string> list = new List<string> { "Ira", "Katya", "Tom", "Kostya" };
                var result = list.Aggregate("",(x,y)=>x+=y.First());
                Console.WriteLine(result);
            }
...

            {
                ...
                int k = 6;
                List<string> list = new List<string> { "IRA001", "1KA1TY1A", "1T1O1M", "KOSTY1" };
                var result = list.Where(g => (g.Length == k) && char.IsDigit(g.Last())).OrderBy(g => g);
            }

            {
                ...
                int D = 5;
                int K = 3;
                List<int> list = new List<int>() { 1, 2, 5, 7, 8, 3, 5, 11, 2241, 67 };

                var l = true;
                var i = -1;
                var result = list.Where(x => l = (x < D) && l).Union(list.Where(x => ++i > K)).OrderBy(x => x);
...

            {
                ...
                //TODO
                List<string> list = new List<string>() { "War","World","Animal","Car","Second" };

                var result = list.Select(i => i.Length % 2 == 0 ? i.ToCharArray().First() :
                i.ToCharArray().Last()).OrderByDescending(i => i);
...

            {
                ...
                List<int> A = new List<int>() { 1, 5, 4, 7, 9, 12,2};
                List<int> B = new List<int>() { 3, 111, 43, 71, 99, 6, 24 };
                int K1 = 5;
                int K2 = 37;

                var result = A.Where(i => i > K1).Select(i => i).Union(B.Where(i => i < K2)).Select(i => i);
            ...
            }
            {
                ...
                List<string> A = new List<string>() { "I1R1A1", "K1O1S1", "T1O4M5" };
                List<string> B = new List<string>() { "K1ATY1", "BO897B", "CY7821P1H123E44R" };

                var res = A.Join(B, x => x.Length, y => y.Length, (x, y) => x + ":" + y)
                    .OrderBy(i => i.Split(':')[0]).ThenByDescending(i =>i.Split(':')[1]);
...

            {
...

                {
                    ...
                    var abiturients = new List<Student>
                    {
                        new Student { Surname = "Jaws", YearApplying = 1999 ,NumberSchool=142},
                        new Student { Surname = "Singing in the Rain", YearApplying = 2001,NumberSchool=142 },
                        new Student { Surname = "Some like it Hot", YearApplying = 2000,NumberSchool=113 },
                        new Student { Surname = "The Wizard of Oz", YearApplying = 2001,NumberSchool=56 },
                        new Student { Surname = "It’s a Wonderful Life", YearApplying = 1999,NumberSchool=289 },
                        new Student { Surname = "American Beauty", YearApplying =2001 ,NumberSchool=14},
                        new Student { Surname = "High Fidelity", YearApplying = 2000,NumberSchool=5},
                        new Student { Surname = "The Usual Suspects", YearApplying = 2000,NumberSchool=1 }
                    };

                    var result = from s in abiturients
                                 group s by s.YearApplying into g
                                 select new
                                 {
                                     Year = g.Key,
                                     Schools = g.Count(),
                                     Abits = from a in g select a
                                 };
